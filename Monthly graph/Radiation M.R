...
DATESR<-function(start_D,end_D,Number_year,STATION,DATATYPE1,DATATYPE2,DATATYPE3)
{
...
PLOTR<- function(startDay,endDay,numberYear,station)
{
  Data10years_df <-   as.tibble(DATESR(start_D = startDay, end_D = endDay, Number_year =  numberYear, STATION =station, 5,2,1)$LTNewdata_df)
  head(Data10years_df)
  start_D2 <- as.Date(startDay) - years(numberYear)
  print(start_D2)
  
  dataperiod_df <- as.tibble(DATESR(start_D = startDay, end_D = endDay, Number_year =  numberYear, STATION =station, 5,2,1)$Newdata_df)
  ...
  p3 <<- ggplot(data = Data10yearsF_df,aes(x=MONTH,y=AVERAGE,fill=YEAR)) +
