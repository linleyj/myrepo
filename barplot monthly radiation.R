...
barplot.fun <- function(start_date,end_date,data_types,stations){
  ...
  datayear1_df <- Daily.datalist %>% 
    map_df(`[`)
  head(datayear1_df)
  names(datayear1_df)
  
  names(datayear1_df)[names(datayear1_df)=="Date(local)"] <- "DDate"
  names(datayear1_df)[names(datayear1_df)=="Amount(MJ/m2)"] <- "Amount"
...
barplot.fun(start_date="2010-10-01 00",end_date = "2013-09-29 00",data_types = cf_datatype(5, 2, 1),stations = cf_station(3925))
